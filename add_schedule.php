    ...
    <section id="add">
        <div class="container">
            ...
            <a href="admin.php"><img src="img/left.svg" alt="" class="admin_arrow" style="height: 40px; width: 40px;"></a>
                <?php
                $scheduls = mysqli_query($connect, "SELECT * FROM `Schedule`");
                ...
                if ($scheduls) {
                    ...
                    while ($schedule = mysqli_fetch_assoc($scheduls)) { 
                        // Вывод данных в строках таблицы
                        echo "<tr>";
                        echo "<td>" . $schedule['Id_schedule'] . "</td>";
                        echo "<td>" . $schedule['Date'] . "</td>";
                        echo "<td>" . $schedule['Time'] . "</td>";
                        echo "<td>" . $schedule['Id_training'] . "</td>";
                        echo "<td>" . $schedule['Id_trainer'] . "</td>";
                        echo "<td>" . $schedule['Id_room'] . "</td>";
                        echo "<td><a href='add_schedule.php?id=" . $schedule['Id_schedule'] . "'>Изменить</a> | <a href='php/vendor_schedule/delete.php?id=" . $schedule['Id_schedule'] . "'>Удалить</a></td>"; // Кнопки "Изменить" и "Удалить"
                        echo "</tr>";
                    }
                    echo "</tbody>";
                    echo "</table>";
                ...
                <div class="inputbox" style="display:flex;">
                    <form class="admin_form" style="margin-right: 250px;" action="php/vendor_schedule/create.php" method="POST">
                        <h3 class="admin_form_title">Добавить данные:</h3>
                        <p class="admin_form_text">Дата:</p>
                        <input type="date" name="Date">
                        <p class="admin_form_text">Время:</p>
                        <input type="time" name="Time">
                        <p class="admin_form_text">Идентификатор тренировки:</p>
                        <input type="Id_training" name="Id_training">
                        <p class="admin_form_text">Идентификатор тренера:</p>
                        <input type="Id_trainer" name="Id_trainer">
                        <p class="admin_form_text">Идентификатор зала:</p>
                        <input type="Id_room" name="Id_room"> <br><br>
                    ...
                    <?php
                        ...
                        if (isset($_GET['id'])) {
                            ...
                            $schedule = mysqli_query($connect, "SELECT * FROM `Schedule` WHERE `Id_schedule` = '$id'");
                            $schedule = mysqli_fetch_assoc($schedule);

                            if ($schedule) {
                                echo "<form class='admin_form' action='php/vendor_schedule/update.php' method='POST'>"; 
                                echo "<h3 class='admin_form_title'>Изменить данные:</h3>";
                                echo "<input type='hidden' name='id' value='" . $schedule['Id_schedule'] . "'>"; 

                                echo "<p class='admin_form_text'>Дата:</p>";
                                echo "<input type='date' name='Date' value='" . $schedule['Date'] . "'>";

                                echo "<p class='admin_form_text'>Время:</p>";
                                echo "<input type='time' name='Time' value='" . $schedule['Time'] . "'>";

                                echo "<p class='admin_form_text'>Идентификатор тренировки:</p>";
                                echo "<input type='text' name='Id_training' value='" . $schedule['Id_training'] . "'>";

                                echo "<p class='admin_form_text'>Идентификатор тренера:</p>";
                                echo "<input type='text' name='Id_trainer' value='" . $schedule['Id_trainer'] . "'>";

                                echo "<p class='admin_form_text'>Идентификатор зала:</p>";
                                echo "<input type='text' name='Id_room' value='" . $schedule['Id_room'] . "'><br> <br>";
